...
def get_dataset(args, algorithm, dataset, num_labels, num_classes, logger, data_dir='./data', include_lb_to_ulb=True):
    """
    create dataset
    ...
    if dataset == "eurosat":
        ...
        lb_dset, ulb_dset, eval_dset, bd_dset = get_cifar(args, algorithm, dataset, num_labels, num_classes, logger, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
        test_dset = None
    elif dataset == 'svhn':
        lb_dset, ulb_dset, eval_dset, bd_dset = get_svhn(args, algorithm, dataset, num_labels, num_classes, logger, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
        test_dset = None
    elif dataset == 'stl10':
        lb_dset, ulb_dset, eval_dset, bd_dset = get_stl10(args, algorithm, dataset, num_labels, num_classes, logger, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
        test_dset = None
    ...
    dataset_dict = {'train_lb': lb_dset, 'train_ulb': ulb_dset, 'eval': eval_dset, 'test': test_dset, 'bd_test': bd_dset}
    return dataset_dict
