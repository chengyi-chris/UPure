...
class AlgorithmBase:
    ...
    def set_data_loader(self):
        ...
        loader_dict["bd_test"] = get_data_loader(
            self.args,
            self.dataset_dict["bd_test"],
            self.args.eval_batch_size,
            # make sure data_sampler is None for evaluation
            data_sampler=None,
            num_workers=self.args.num_workers,
    ...
    def evaluate(self, eval_dest="eval", out_key="logits", return_logits=False):
        ...
        ########################## Evaluation for backdoor data ##########################
        
        bd_loader = self.loader_dict['bd_test']
#         total_loss = 0.0
#         total_num = 0.0
        y_true = []
        y_pred = []
#         y_logits = []
        ...
        target_class = 7
        with torch.no_grad():
            for data in bd_loader:
                x = data["x_lb"]
                y = data["y_lb"]
                y = torch.full(y.size(), target_class)
...
#         y_logits = np.concatenate(y_logits)
        ...
        asr_success = 0
        asr_total = len(y_true)
        
        top1 = accuracy_score(y_true, y_pred)
        is_target_class = (y_pred == y_true)
        asr_success = is_target_class.sum().item()
        asr = asr_success / asr_total

        self.print_fn("Backdoor Eval ASR: {:.4f}%".format(asr*100))
        self.print_fn("Eval ASR:"+ str(top1))
        

        ########################## Evaluation for benign data ##########################
